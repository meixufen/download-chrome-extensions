...
ext_download_url_base = 'https://clients2.google.com/service/update2/crx?' \
    + 'response=redirect&prodversion=49.0&x=id%3Dohmmkhmmmpcnpikjeljgnaoabkaalbgc%26installsource%3Dondemand%26uc'
...
def ext_info_add_list(extinfo = {}):
    ...
    if get_int(extinfo.get('users')) < conf['more_then_user_num']:
        return False
    print('[*] id : ' + extid)
    path = conf['tmp_path']
    if extid:
